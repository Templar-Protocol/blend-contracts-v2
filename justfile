...
# Emit the finite fuzz-target matrix from the registry.
verify-list kind selection="all":
    ...
    jq -e '
        .version == 1 and
        ((keys | sort) == ["fuzz", "version"]) and
        (.fuzz | type == "array" and length == 7) and
        ([.fuzz[].target] | length == (unique | length)) and
        (all(.fuzz[]; (.target | test("^fuzz_[a-z0-9_]+$")) and (.contracts | length > 0) and (.entrypoints | length > 0)))
