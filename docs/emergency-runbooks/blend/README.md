...
A companion collection lives in [`../templar/`](../templar/) for the Templar
Protocol on NEAR (immutable markets, registry, LST and proxy oracle adapters,
NEAR vault runtime) and its surrounding NEAR ecosystem. Incidents that cross
both ecosystems route via both collections' Safe Chains — see the top-level
[`../README.md`](../README.md) for the two-collection overview.
