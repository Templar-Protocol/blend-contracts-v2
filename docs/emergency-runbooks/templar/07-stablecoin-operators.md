...
|-------|--------|-------------|
| **Critical (P0)** | Confirmed reserve discrepancy; peg deviation exceeding your issuer's published P0 threshold, measured against your published reference price over your published evaluation window; **operator-executable fallback if no policy is published**: peg deviation > 100 bps (1%) sustained for > 15 minutes, measured against the median mid-price across the top-3 CEX venues with liquidity in the asset (issuer to publish its own binding thresholds and reference; this is a fallback so responders can act, not a substitute for policy); authority key capture. | Pause issuance and redemption. Notify NEAR Foundation immediately; Foundation pages every Templar dev-team contact, vault curator holding your stablecoin, bridge operator moving it, and the NEAR Intents team. Expect vault curators to Sentinel-pause vaults holding the stablecoin. Templar dev team will halt bots defensively. |
| **High (P1)** | Single-source reserve report missing; large unexplained mint / redeem flow; peg deviation approaching but under the P0 threshold and trending; **operator-executable fallback if no policy is published**: peg deviation 25–100 bps sustained for > 15 minutes, measured against the same reference as the P0 row (issuer to publish its own binding thresholds and reference). | Investigate; brief NEAR Foundation; pre-warn curators and dev team to be ready to act. |
