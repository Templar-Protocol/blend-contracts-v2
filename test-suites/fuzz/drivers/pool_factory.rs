...
pub fn run(operations: &[Operation], mode: Mode) -> RunReport {
    ...
    for (index, operation) in operations.iter().enumerate() {
        ...
        // Main's pool constructor requires a zero backstop take rate
        // (`execute_initialize`), and `deploy_v2` surfaces the failing
        // constructor as a host `Context` error instead of a contract error.
        // The strict classifier therefore fuzzes only the zero success rate
        // and factory-rejected rates at or above the 10_000_000 boundary;
        // the (0, 10_000_000) band is outside the contract-error domain.
        let take_rate = if operation.flags & 1 == 0 {
            0
        } else if operation.flags & 2 == 0 {
            10_000_000
        } else {
            10_000_001
        };
        ...
        let expected = take_rate == 0 && (2..=60).contains(&max_positions) && min_collateral >= 0;
        let mut salt = [0u8; 32];
        salt[0] = index as u8;
