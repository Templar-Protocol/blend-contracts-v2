...
fn invariants(
    ...
    assert_eq!(emissions.is_some(), configured.is_some());
    if let Some(id) = configured {
        let emissions = emissions.unwrap();
        assert_eq!(emissions.len(), 1);
        assert_eq!(emissions.get(id), Some(1_0000000));
    }
...
pub fn replay_generated(operations: &[(u8, u8, u8, u64, u32)]) {
    ...
    for (prefix, &(op, user, reserve, amount, time)) in operations.iter().enumerate() {
        let run = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            match op {
                ...
                10 => {
                    ...
                    let blocked_emissions = has_default
                        && positions.collateral.iter().any(|(index, amount)| {
                            amount > 0
                                && (configured == Some(index * 2 + 1)
                                    || seeded.contains(&(index * 2 + 1)))
                        });
                    if auction {
                ...
                11 => {
                    ...
                    let custody = pool
                        ...
                        .unwrap_or(0);
                    let contradictory =
                        configured == Some(id * 2 + 1) || seeded.contains(&(id * 2 + 1));
                ...
                13 => {
                    // Contradictory b-token emissions configured through the retained
                    // admin route; later prefixes must hold exact configured values.
                    let id = f.pools[0].reserves[&asset];
            ...
            invariants(&f, &cash, foreign_blnd, configured, &seeded, seed_eps);
            traps(&f, user);
        }));
