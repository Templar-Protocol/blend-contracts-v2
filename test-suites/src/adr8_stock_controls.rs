...
/// executes all fourteen controls against supplied artifacts.
pub fn run_stock_controls(
    ...
    class7_no_denominator_witness(&root, base_pool, base_backstop, fork_pool);
}
...
fn class7_default_control(root: &Path, base_pool: &[u8], base_backstop: &[u8], fork_pool: &[u8]) {
    direct_default_control(
        root,
        base_pool,
        base_backstop,
        fork_pool,
        "pool_direct_supplier_default",
        false,
    );
}

/// ADR 0008 declared deviation witness: the user's same-reserve claim is the entire
/// `b_supply` and covers only half the debt. Stock assigns the gross liability to the
/// backstop; the fork burns the whole claim, then forgives the residual at an unchanged
/// `b_rate` because no supplier claim remains (stock's per-event update would divide by
/// zero). This is asserted explicitly, never absorbed by the oracle.
fn class7_no_denominator_witness(
    root: &Path,
    base_pool: &[u8],
    base_backstop: &[u8],
    fork_pool: &[u8],
    ...
    direct_default_control(
        ...
        base_backstop,
        fork_pool,
        "pool_direct_default_no_denominator",
        true,
    );
}

fn direct_default_control(
    root: &Path,
    base_pool: &[u8],
    base_backstop: &[u8],
    fork_pool: &[u8],
    ...
    if no_denominator {
        // Host preparation: one liability reserve whose whole b_supply is this user's
        // claim, sized to cover only half of the debt.
        let (index, amount) = liabilities
            .iter()
            .next()
            .expect("prepared user must hold a liability");
        let (token_index, _) = f.pools[0]
            .reserves
            .iter()
            .find(|(_, id)| **id == index)
            .unwrap();
        let key = pool::PoolDataKey::ResData(f.tokens[*token_index].address.clone());
        f.env.as_contract(&f.pools[0].pool.address, || {
            let mut data: pool::ReserveData = f.env.storage().persistent().get(&key).unwrap();
            let debt_assets = (amount * data.d_rate + SCALAR_12 - 1) / SCALAR_12;
            let claim = ((debt_assets / 2) * SCALAR_12 + data.b_rate - 1) / data.b_rate;
            assert!(claim > 0 && claim * data.b_rate / SCALAR_12 < debt_assets);
            positions.supply.set(index, claim);
            data.b_supply = claim;
    ...
    // Capture before any further fork-env invocation: the SDK keeps only the last call's events.
    let fork_events = fork_env.events().all();
    ...
    for (index, amount) in liabilities.iter() {
        ...
        if defaulted > 0 && remaining_b_supply == 0 {
            // ADR 0008 declared deviation: no supplier claim remains after setoff, so the
            // residual is forgiven at an unchanged b_rate (stock would divide by zero).
            deviation_hit = true;
            assert_eq!(
                fork.b_rate, base.b_rate,
                "declared no-denominator deviation: b_rate must be preserved"
            );
        } else {
            let loss = if defaulted > 0 {
        ...
        // Every setoff is explained by exactly one `debt_setoff(asset)` event carrying
        // the burned b-tokens and repaid d-tokens.
        let setoff_event = svec![
            &fork_env,
            (
                fork_id.clone(),
                (
                    soroban_sdk::Symbol::new(&fork_env, "debt_setoff"),
                    sc_address(&fork_env, asset),
                )
                    .into_val(&fork_env),
                (burned, repaid).into_val(&fork_env),
            )
        ];
        let observed = fork_events
            .iter()
            .filter(|event| svec![&fork_env, event.clone()] == setoff_event)
            .count();
        assert_eq!(
            observed,
            usize::from(burned > 0),
            "debt_setoff event attribution"
        );
        if burned > 0 {
            if claim == burned {
    ...
    assert_eq!(
        deviation_hit, no_denominator,
        "no-denominator witness must exercise the declared deviation exactly when prepared"
    );
